...
for i=0:9
    for j=0:9
        s = sprintf('voice_data2/train/%i%i.wav',i,j);%把格式化的数据写入字符串中
...
for i=0:9
    for j=0:9
        s = sprintf('voice_data2/test/%i0%i.wav',i,j);
        [s1 fs1] = audioread(s);
        v = mfcc(s1, fs1);
        a= vqlbg(v, k); 
...
net=newff(minmax(P_train),[200,10],{'tansig' 'tansig'},'traingda');
